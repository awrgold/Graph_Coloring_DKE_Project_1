...
//					state.states[GameState.INGAME] = new GameMode(state, GraphUtil.readGraphFromFile(file)); //Start the actual game. HOW to implement feedback: what happens if we cannot compute the chromatic number, or the file was corrupt??
...
//							state.states[GameState.INGAME] = new GameMode(state, GraphUtil.generateRandomGraph(m, n)); //Start the actual game
//							state.setState(GameState.INGAME);
//						}
...
//				state.states[GameState.INGAME] = new GameMode(state, new Graph(CircleSolver.getVertexObjects(CircleSolver.circlesolver (170, 0, 270, 14, false)), new int[][]{})); //Here we test out CircleSolver
//				state.setState(GameState.INGAME);
//			}
//
//			clickedVertex = -1;
